...
static void adc_config_channel(bool left)
{
...
static bool sense_key(pin_t row, bool on, int col)
{
    bool key_down = false;
...
bool matrix_scan_custom(matrix_row_t* raw)
{
    ...
    for (int row = 0; row < MATRIX_ROWS; row++) {
        ...
        for (int col = 0; col < MATRIX_COLS; col++) {
            ...
            adc_config_channel(col<8);

            if (sense_key(custom_row_pins[row], (last_row_value&col), col)) {
