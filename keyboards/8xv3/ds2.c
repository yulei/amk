...
void render_task(render_t* render)
{
    ...
    if (filling) {
        ...
        if(gc9107_fill_ready(&gc9107_driver)){
            filling = false;
        } else {
            return;
        }
    ...
    if ( need_refresh(render->ticks, render->delay)) {
        ...
        gc9107_fill_rect(&gc9107_driver, render->x, render->y, render->width, render->height, render->buf, render->buf_size);
        filling = true;
