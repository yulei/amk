...
swap32 (unsigned int source)
{
   ...
   buffer[0] = ((uint8_t *) &source)[3];
   buffer[1] = ((uint8_t *) &source)[2];
   buffer[2] = ((uint8_t *) &source)[1];
   buffer[3] = ((uint8_t *) &source)[0];
...
swap16 (unsigned short source)
{
   ...
   buffer[0] = ((uint8_t *) &source)[1];
   buffer[1] = ((uint8_t *) &source)[0];
...
SNSS_ReadBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
   ...
   //if (fp.read(headerBytes, 12) != 12)
   UINT readed = 0;
   if (f_read(&fp, header, 12, &readed) != FR_OK)
   {
...
SNSS_WriteBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
   ...
   headerBytes[4] = ((uint8_t *) &tempInt)[0];
   headerBytes[5] = ((uint8_t *) &tempInt)[1];
   headerBytes[6] = ((uint8_t *) &tempInt)[2];
   headerBytes[7] = ((uint8_t *) &tempInt)[3];

   tempInt = swap32 (header->blockLength);
   headerBytes[8] = ((uint8_t *) &tempInt)[0];
   headerBytes[9] = ((uint8_t *) &tempInt)[1];
   headerBytes[10] = ((uint8_t *) &tempInt)[2];
   headerBytes[11] = ((uint8_t *) &tempInt)[3];

   //if (fwrite (headerBytes, 12, 1, snssFile->fp) != 1)
   //if (fp.write (headerBytes, 12) != 12)
   UINT writen = 0;
   if (f_write (&fp, headerBytes, 12, &writen) != FR_OK)
   {
...
SNSS_ReadFileHeader (SNSS_FILE *snssFile)
{
   ...
   //if (fp.read(snssFile->headerBlock.tag, 4) != 4)
   UINT readed = 0;
   if (f_read(&fp, snssFile->headerBlock.tag, 4, &readed) != FR_OK)
   {
      return SNSS_READ_FAILED;
   }
   ...
   //if (fp.read(&snssFile->headerBlock.numberOfBlocks, 4) != 4)
   if (f_read(&fp, &snssFile->headerBlock.numberOfBlocks, 4, &readed) != FR_OK)
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_WriteFileHeader (SNSS_FILE *snssFile)
{
   ...
   writeBuffer[4] = ((uint8_t *) &tempInt)[0];
   writeBuffer[5] = ((uint8_t *) &tempInt)[1];
   writeBuffer[6] = ((uint8_t *) &tempInt)[2];
   writeBuffer[7] = ((uint8_t *) &tempInt)[3];

   //if (fwrite (writeBuffer, 8, 1, snssFile->fp) != 1)
   //if (fp.write (writeBuffer, 8) != 8)
   UINT writen = 0;
   if (f_write(&fp, writeBuffer, 8, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
...
SNSS_CloseFile (SNSS_FILE **snssFile)
{
   ...
   {
      prevLoc = f_tell(&fp);//fp.position(); //ftell((*snssFile)->fp);
...
SNSS_GetNextBlockType (SNSS_BLOCK_TYPE *blockType, SNSS_FILE *snssFile)
{
   ...
   //if (fp.read(tagBuffer, TAG_LENGTH) != TAG_LENGTH)
   UINT readed = 0;
   if (f_read(&fp, tagBuffer, TAG_LENGTH, &readed) != FR_OK)
   {
      return SNSS_READ_FAILED;
   }
   ...
   //if (! fp.seekCur (-TAG_LENGTH))
   UINT pos = f_tell(&fp);
   if (FR_OK != f_lseek(&fp, pos-TAG_LENGTH))
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_SkipNextBlock (SNSS_FILE *snssFile)
{
   ...
   UINT cur = f_tell(&fp);
   //if (! fp.seekCur(TAG_LENGTH + sizeof (unsigned int)))
   if (FR_OK != f_lseek(&fp, cur + TAG_LENGTH+sizeof(unsigned int)))
   {
      return SNSS_READ_FAILED;
   }
   ...
   //if (fp.read(&blockLength, 4) != 4)
   UINT readed = 0;
   if (f_read(&fp, &blockLength, 4, &readed) != FR_OK)
   {
      return SNSS_READ_FAILED;
   }
   ...
   //if (! fp.seekCur(blockLength) )
   cur = f_tell(&fp);
   //if (! fp.seekCur(TAG_LENGTH + sizeof (unsigned int)))
   if (FR_OK != f_lseek(&fp, cur + blockLength))
   {
      return SNSS_READ_FAILED;
   }
...
{
   uint8_t* blockBytes = (uint8_t*) emu_Malloc(BASE_BLOCK_LENGTH);
   if (!blockBytes) abort();
   ...
   //if (! fp.read(blockBytes, MIN (header.blockLength, BASE_BLOCK_LENGTH)))
   UINT readed = 0;
   if (FR_OK != f_read(&fp, blockBytes, MIN (header.blockLength, BASE_BLOCK_LENGTH), &readed))
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_WriteBaseBlock (SNSS_FILE *snssFile)
{
   ...
   blockBytes[0x5] = ((uint8_t *) &tempShort)[0];
   blockBytes[0x6] = ((uint8_t *) &tempShort)[1];
   ...
   blockBytes[0x192D] = ((uint8_t *) &tempShort)[0];
   blockBytes[0x192E] = ((uint8_t *) &tempShort)[1];
   ...
   //if (fp.write (blockBytes, BASE_BLOCK_LENGTH) != BASE_BLOCK_LENGTH)
   UINT writen=0;
   if (f_write (&fp, blockBytes, BASE_BLOCK_LENGTH, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
...
SNSS_ReadVramBlock (SNSS_FILE *snssFile)
{
   ...
   //if (! fp.read (snssFile->vramBlock.vram, MIN (header.blockLength, VRAM_16K)))
   UINT readed = 0;
   if (FR_OK != f_read(&fp, snssFile->vramBlock.vram, MIN (header.blockLength, VRAM_16K), &readed))
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_WriteVramBlock (SNSS_FILE *snssFile)
{
   ...
   //if (fp.write (snssFile->vramBlock.vram, snssFile->vramBlock.vramSize) != snssFile->vramBlock.vramSize)   
   UINT writen = 0;
   if (FR_OK != f_write(&fp, snssFile->vramBlock.vram, snssFile->vramBlock.vramSize, &writen))   
   {
      return SNSS_WRITE_FAILED;
   }
...
SNSS_ReadSramBlock (SNSS_FILE *snssFile)
{
   ...
   //if (fp.read (&snssFile->sramBlock.sramEnabled, 1) != 1)
   UINT readed = 0;
   if (f_read (&fp, &snssFile->sramBlock.sramEnabled, 1, &readed) != FR_OK)
   {
      return SNSS_READ_FAILED;
   }
   ...
   //if (! fp.read (&snssFile->sramBlock.sram, MIN (header.blockLength - 1, SRAM_8K))) 
   readed = 0;
   if (FR_OK != f_read (&fp, &snssFile->sramBlock.sram, MIN (header.blockLength - 1, SRAM_8K), &readed)) 
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_WriteSramBlock (SNSS_FILE *snssFile)
{
   ...
   //if (fp.write (&snssFile->sramBlock.sramEnabled, 1) != 1)
   UINT writen = 0;
   if (f_write (&fp, &snssFile->sramBlock.sramEnabled, 1, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
   
   //   if (fwrite (snssFile->sramBlock.sram, snssFile->sramBlock.sramSize, 1, snssFile->fp) != 1)
   //if (fp.write (snssFile->sramBlock.sram, snssFile->sramBlock.sramSize) != snssFile->sramBlock.sramSize)
   if (f_write (&fp, snssFile->sramBlock.sram, snssFile->sramBlock.sramSize, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
...
SNSS_ReadMapperBlock (SNSS_FILE *snssFile)
{
   ...
   if ((blockBytes = (uint8_t *) emu_Malloc(0x8 + 0x10 + 0x80)) == NULL)
   {
      return SNSS_OUT_OF_MEMORY;
   }

   //if (fread (blockBytes, MIN (0x8 + 0x10 + 0x80, header.blockLength), 1, snssFile->fp) != 1)
   //if (! fp.read (blockBytes, MIN (0x8 + 0x10 + 0x80, header.blockLength)))
   UINT readed=0;
   if (FR_OK != f_read (&fp, blockBytes, MIN (0x8 + 0x10 + 0x80, header.blockLength), &readed))
   {
      emu_Free(blockBytes);
      return SNSS_READ_FAILED;
   }
...
{
   SnssBlockHeader header;
   uint8_t blockBytes[MAPPER_BLOCK_LENGTH];
   unsigned short tempShort;
   int i;
   SNSS_RETURN_CODE returnCode;
   ...
   for (i = 0; i < 4; i++)
   {
      ...
      blockBytes[(i * 2) + 0] = ((uint8_t *) &tempShort)[0];
      blockBytes[(i * 2) + 1] = ((uint8_t *) &tempShort)[1];
   }

   for (i = 0; i < 8; i++)
   {
      tempShort = swap16 (snssFile->mapperBlock.chrPages[i]);
      blockBytes[0x8 + (i * 2) + 0] = ((uint8_t *) &tempShort)[0];
      blockBytes[0x8 + (i * 2) + 1] = ((uint8_t *) &tempShort)[1];
   }
   ...
   //if (fp.write (blockBytes, MAPPER_BLOCK_LENGTH) != MAPPER_BLOCK_LENGTH)
   UINT writen = 0;
   if (f_write (&fp, blockBytes, MAPPER_BLOCK_LENGTH, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
...
SNSS_ReadSoundBlock (SNSS_FILE *snssFile)
{
   ...
   //if (! fp.read (snssFile->soundBlock.soundRegisters, MIN (header.blockLength, 0x16)))
   UINT readed = 0;
   if (FR_OK != f_read (&fp, snssFile->soundBlock.soundRegisters, MIN (header.blockLength, 0x16), &readed))
   {
      return SNSS_READ_FAILED;
   }
...
SNSS_WriteSoundBlock (SNSS_FILE *snssFile)
{
   ...
   //if (fp.write (snssFile->soundBlock.soundRegisters, SOUND_BLOCK_LENGTH) != SOUND_BLOCK_LENGTH)
   UINT writen=0;
   if (f_write (&fp, snssFile->soundBlock.soundRegisters, SOUND_BLOCK_LENGTH, &writen) != FR_OK)
   {
      return SNSS_WRITE_FAILED;
   }
