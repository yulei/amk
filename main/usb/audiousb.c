...
uint32_t current_sample_rate = CFG_TUD_AUDIO_FUNC_1_MAX_SAMPLE_RATE;
int8_t mute[CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX+1];       // +1 for master channel 0
int16_t volume[CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX+1];    // +1 for master channel 0
int16_t audio_buf[CFG_TUD_AUDIO_FUNC_1_EP_OUT_SW_BUF_SZ];
...
static bool tud_audio_clock_get_request(uint8_t rhport, audio_control_request_t const *request)
{
    if (request->bControlSelector == AUDIO_CS_CTRL_SAM_FREQ) {
        if (request->bRequest == AUDIO_CS_REQ_CUR) {
            ...
            {
                rangef.subrange[i].bMin = current_sample_rate;
                rangef.subrange[i].bMax = current_sample_rate;
...
bool tud_audio_rx_done_pre_read_cb(uint8_t rhport, uint16_t n_bytes_received, uint8_t func_id, uint8_t ep_out, uint8_t cur_alt_setting)
{
    ...
    audio_debug("Audio data received: ep=%d, size=%d\r\n", ep_out, n_bytes_received);

    int32_t readed = tud_audio_read(audio_buf, n_bytes_received);

    audio_debug("Audio data readed: ep=%d, size=%ld\r\n", ep_out, readed);
    return true;
}
