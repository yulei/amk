...
static HSV SOLID_REACTIVE_WIDE_math(rgb_matrix_state_t *state, HSV hsv, int16_t dx, int16_t dy, uint8_t dist, uint16_t tick) {
...
#            ifdef ENABLE_RGB_MATRIX_SOLID_REACTIVE_MULTIWIDE
bool SOLID_REACTIVE_MULTIWIDE(rgb_matrix_state_t *state) { return effect_runner_reactive_splash(0, state, &SOLID_REACTIVE_WIDE_math); }
#            endif
