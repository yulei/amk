...
bool TYPING_HEATMAP(rgb_matrix_state_t *state) {
    // Modified version of RGB_MATRIX_USE_LIMITS to work off of matrix row / col size
    uint8_t led_min = RGB_MATRIX_LED_PROCESS_LIMIT * state->rgb_effect_params.iter;
    uint8_t led_max = led_min + RGB_MATRIX_LED_PROCESS_LIMIT;
    if (led_max > sizeof(state->g_rgb_frame_buffer)) led_max = MATRIX_ROWS*MATRIX_COLS;//sizeof(g_rgb_frame_buffer);
    ...
    for (int i = led_min; i < led_max; i++) {
        ...
        uint8_t val = state->g_rgb_frame_buffer[row][col];
        ...
        if (decrease_heatmap_values) {
            state->g_rgb_frame_buffer[row][col] = qsub8(val, 1);
        }
    }

    return led_max < (MATRIX_ROWS*MATRIX_COLS);//sizeof(g_rgb_frame_buffer);
}
