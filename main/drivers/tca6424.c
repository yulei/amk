...
{
    if (!i2c_inst) {
        i2c_inst = ak_i2c_init(TCA6424_I2C_ID);
...
{
    amk_error_t error = ak_i2c_write_reg(i2c_inst, tca_addr, port, &data, 1, TIMEOUT);
...
{
    uint8_t data = 0;
    amk_error_t error = ak_i2c_read_reg(i2c_inst, tca_addr, port, &data, 1, TIMEOUT);
    if (error != AMK_SUCCESS) {
