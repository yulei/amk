...
void init_driver(is31fl3731_driver_t *driver)
{
    ...
    for (int i = 0; i < driver->is31.led_num; i++) {
        ...
        uint8_t reg_r = (r - PWM_REG) / 8;
        uint8_t reg_g = (g - PWM_REG) / 8;
        uint8_t reg_b = (b - PWM_REG) / 8;
        uint8_t bit_r = (r - PWM_REG) % 8;
        uint8_t bit_g = (g - PWM_REG) % 8;
        uint8_t bit_b = (b - PWM_REG) % 8;
