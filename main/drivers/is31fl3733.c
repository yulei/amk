...
void is31fl3733_update_buffers(i2c_led_t *driver)
{
    ...
    if (is31->pwm_dirty) {
        ...
        ak_i2c_write_reg(i2c_inst, driver->addr, COMMAND_WRITE_LOCK_REG, &data, 1, TIMEOUT);

        // select pwm page
        data = PAGE_PWM;
        ak_i2c_write_reg(i2c_inst, driver->addr, COMMAND_REG, &data, 1, TIMEOUT);

        ak_i2c_send(i2c_inst, driver->addr, is31->pwm_buffer, PWM_BUFFER_SIZE+1, TIMEOUT);
...
{
    if (!i2c_inst) {
        i2c_inst = ak_i2c_init(IS31FL3733_I2C_ID);
    }
    ...
    // unlock 
    uint8_t data = COMMAND_UNLOCK;
    ak_i2c_write_reg(i2c_inst, driver->i2c_led.addr, COMMAND_WRITE_LOCK_REG, &data, 1, TIMEOUT);
    ...
    for (int i = 0; i < CONTROL_BUFFER_SIZE; i++) {
        ak_i2c_write_reg(i2c_inst, driver->i2c_led.addr, i, &data, 1, TIMEOUT);
    }
    ...
    // set gloabl current
    data = 0xFF;
    ak_i2c_write_reg(i2c_inst, driver->i2c_led.addr, FUNCTION_GLOBAL_CURRENT_REG, &data, 1, TIMEOUT);

    // enable the chip
    data = 1;
    ak_i2c_write_reg(i2c_inst, driver->i2c_led.addr, FUNCTION_CONFIGURATION_REG, &data, 1, TIMEOUT);
...
static void uninit_driver(i2c_led_t *driver)
{
    ...
    // disable the chip
    data = 0;
    ak_i2c_write_reg(i2c_inst, driver->addr, FUNCTION_CONFIGURATION_REG, &data, 1, TIMEOUT);
}
