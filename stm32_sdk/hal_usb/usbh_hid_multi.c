...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost, uint8_t itf);
static USBH_StatusTypeDef USBH_HID_InterfaceDeInit(USBH_HandleTypeDef *phost, uint8_t itf);
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost, uint8_t itf);
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost, uint8_t itf);
static USBH_StatusTypeDef USBH_HID_SOFProcess(USBH_HandleTypeDef *phost, uint8_t itf);
static void  USBH_HID_ParseHIDDesc(HID_DescTypeDef *desc, uint8_t *buf, uint8_t irf);
...
static USBH_StatusTypeDef hid_multi_init(USBH_HandleTypeDef *phost)
{
    ...
    HID_HandleTypeDef *hid_current = &hid_multi;

    memset(hid_current, 0, sizeof(HID_HandleTypeDef));
    phost->pActiveClass->pData = hid_current;

    USBH_CfgDescTypeDef *pcfg = &phost->device.CfgDesc;
    for (int i = 0; i < pcfg->bNumInterfaces; i++) {
        USBH_InterfaceDescTypeDef *pitf = &pcfg->Itf_Desc[i];
        if (pitf->bInterfaceClass == USB_HID_CLASS) {
            USBH_HID_InterfaceInit(phost, i);
            status = USBH_OK;
        }
    }
    return status;
...
static USBH_StatusTypeDef hid_multi_deinit(USBH_HandleTypeDef *phost)
{
    HID_HandleTypeDef *hid_current = (HID_HandleTypeDef*)phost->pActiveClass->pData;
    for (int i = 0; i < ITF_MAX; i++) {
        if (hid_current->itfs[i].valid != 0) {
